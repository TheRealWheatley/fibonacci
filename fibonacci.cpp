...
vector<int> factors(const int x, const vector<int> v){
	vector<int> facts;
...
int main(){
	while (true){
		...
		cout << "Gimme two ints and a stop pal" << endl;
		cin >> a >> b >> n;
		fibonacci(a, b, vfibonacci, n);
		//include factors?
		char choice;
		cout << "Include factors?";
		cin >> choice;
		switch (choice){
			case 'y':
			...
			for (int i = 0; i < (vfibonacci.size() - 1); ++i){
				...
				vector<int> facts = factors(vfibonacci[i], subvfib);
				if (facts.size() > 0){
					cout << "factors of " << vfibonacci[i] << ": (";
